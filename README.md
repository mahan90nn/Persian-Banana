...
rcon <server> <password> RCON یک سرور متصل می‌شود

brutrcon <سرور> <فایل> رمزهای عبور فایل داده شده را برای اتصال به RCON امتحان می‌کند

fuzz <website <file> <threads> مثال: example.com/FUZZ یا FUZZ.example.com

sendcmd <username> <server> <file> یک ربات ارسال می‌کند که لیستی از دستورات را از یک فایل اجرا می‌کند
